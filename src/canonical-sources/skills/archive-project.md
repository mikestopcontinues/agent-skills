...
```bash
__SKILL_HOME__/scripts/doc-check-links.sh
```
...
3. If promote: invoke `/create-decision --global <name>` style flow
   (or directly scaffold via `__SKILL_HOME__/scripts/yolo new decision
...
3. If move: scaffold via `__SKILL_HOME__/scripts/yolo new note --global
