...
const canonRoot = resolve(repoRoot, 'src/canonical-sources');
...
/**
 * Walk `src/canonical-sources/` and `src/scripts/`, importing every `define*`
 * module. The filesystem is the registry — no static import list. Throws if any
 * module fails to load or fails its factory validation, or if a skill / agent /
 * hook `name` does not match its filename.
 */
