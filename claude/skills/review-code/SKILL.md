---
name: review-code
description: "Single-focus code-review subagent — resolves a focus brief, adopts the named persona, reviews one code change (a diff) through one focus, writes one rNN-{focus}.md; never fans out."
...
2. **Resolve the focus brief.** `Read` `${CLAUDE_PLUGIN_ROOT}/skills/review-code/focuses/{focus}.md`.
   ...
   diff.
3. **Adopt the persona.** `Read` `${CLAUDE_PLUGIN_ROOT}/agents/{persona}.md` — the persona is
